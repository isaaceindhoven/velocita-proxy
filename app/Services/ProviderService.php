...
class ProviderService
{
    ...
    public function createProviderCache(ProviderReference $providerRef, \Closure $dataCallback = null)
    {
        ...
        $remoteProviderURL = config("repositories.{$repo->name}.url") . $providerPath;
        ...
        $storage->put(sprintf('repo/%s/%s.json', $repo->name, $providerName), $providerData);
     ...
     */
    protected function findProviderInProviderInclude(Repository $repo, ProviderInclude $providerInclude, string $namespace, string $package): ?ProviderReference
    {
     ...
     * @return \App\Composer\ProviderReference|null
     */
    public function findProviderInRepository(Repository $repo, string $namespace, string $package): ?ProviderReference
    {
